...
- **Wiki search lifecycle also lives in service.** `service/wiki_search.go` owns repo-scoped wiki search documents, asynchronous put/move/delete/label indexing, label-filtered recall, label-name/description lexical boosting, substring fallback, semantic ranking, snippet generation, and the explicit `ReindexWikiSearch`/`ReindexAllWikiSearch` backfill path used by the `wiki-reindex` CLI command.
