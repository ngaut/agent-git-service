...
- **Wiki search lifecycle also lives in service.** `service/wiki_search.go` owns repo-scoped wiki search candidate indexes, asynchronous put/move/delete/label indexing, git-first lexical recall, label-name/description lexical boosting, semantic ranking, stale-row filtering, live result hydration, and the explicit `ReindexWikiSearch`/`ReindexAllWikiSearch` backfill path used by the `wiki-reindex` CLI command.
