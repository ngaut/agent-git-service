...
func TestWiki_CompactHistory_StartsAsyncJob_Issue1472(t *testing.T) {
	...
	update := h.DoRESTJSONWithToken(t, "PUT", "/api/ext/v1/repos/"+full+"/wiki/pages/home", ownerToken, map[string]any{
		"body":    "# Home\n\nSecond version.\n",
		"message": "update home",
		"sha":     currentSHA,
	})
	...
	blocked := h.DoRESTJSONWithToken(t, "POST", "/api/ext/v1/repos/"+full+"/wiki/compact", strangerToken, nil)
	...
	compact := h.DoRESTJSONWithToken(t, "POST", "/api/ext/v1/repos/"+full+"/wiki/compact", ownerToken, map[string]any{})
	...
	after := h.DoREST(t, "GET", "/api/ext/v1/repos/"+full+"/wiki/pages/home/history", nil)
	...
	req := httptest.NewRequest("POST", "/api/ext/v1/repos/"+full+"/wiki/compact", bytes.NewReader([]byte("{")))
...
func TestWiki_CompactHistory_CompletesAfterRequestContextCanceled_Issue1472(t *testing.T) {
	...
	req := httptest.NewRequest("POST", "/api/ext/v1/repos/"+full+"/wiki/compact", bytes.NewReader([]byte("{}"))).WithContext(reqCtx)
	req.Header.Set("Authorization", "token "+ownerToken)
	req.Header.Set("Content-Type", "application/json")
...
func TestWiki_RepairLocks_AdminOnly(t *testing.T) {
	...
	blocked := h.DoRESTJSONWithToken(t, "POST", "/api/ext/v1/admin/wiki/repos/"+full+"/repair-locks", strangerToken, map[string]any{})
	if blocked.Code != http.StatusForbidden && blocked.Code != http.StatusNotFound {
		t.Fatalf("non-admin repair expected 403/404, got %d: %s", blocked.Code, blocked.Body.String())
	}

	fresh := h.DoRESTJSONWithToken(t, "POST", "/api/ext/v1/admin/wiki/repos/"+full+"/repair-locks", ownerToken, map[string]any{})
	assertStatusCode(t, fresh, http.StatusConflict)

	forced := h.DoRESTJSONWithToken(t, "POST", "/api/ext/v1/admin/wiki/repos/"+full+"/repair-locks", ownerToken, map[string]any{"force": true})
