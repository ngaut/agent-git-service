...
func TestWiki_ReadsListsAndIndexesLegacyStoredSlugs_Issue1355(t *testing.T) {
	...
	ctx := context.Background()
	owner := db.User{Login: "wiki-legacy-owner", Name: "wiki-legacy-owner", Type: db.TypeUser}
	if err := svc.DB.Create(&owner).Error; err != nil {
		t.Fatalf("create owner: %v", err)
	}
	if _, err := svc.CreateRepo(ctx, service.CreateRepoInput{
	...
	full := owner.Login + "/wiki-legacy"
	if err := svc.Git.Init(ctx, full+".wiki", "master", false); err != nil {
		t.Fatalf("init wiki repo: %v", err)
	}

	if _, err := svc.Git.WriteFile(ctx, full+".wiki", "master", "Legacy_Page.v2.md", "add legacy page", []byte("# Legacy\n")); err != nil {
	...
	if err != nil {
		t.Fatalf("GetWikiPage legacy: %v", err)
	}
	if page.Slug != "Legacy_Page.v2" {
		t.Fatalf("legacy page slug = %q, want Legacy_Page.v2", page.Slug)
	}
	...
	backlinks, err := svc.ListWikiBacklinks(ctx, full, "Legacy_Page.v2")
	if err != nil {
