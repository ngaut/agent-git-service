...
func TestNewCmdPrs(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			input: `"repo:cli/cli is:pr state:open label:auth author:testadmin in:title blocked"`,
			output: shared.IssuesOptions{
				Query: search.Query{
					ImmutableKeywords: "repo:cli/cli is:pr state:open label:auth author:testadmin in:title blocked",
					Keywords:          []string{},
					Kind:              "issues",
					Limit:             30,
					Qualifiers:        search.Qualifiers{Type: "pr"},
