...
func NewCmdCode(f *cmdutil.Factory, runF func(*CodeOptions) error) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(c *cobra.Command, args []string) error {
			...
			// Validate --match flag: for code search, only one value (path OR file) is allowed
			if len(opts.Query.Qualifiers.In) > 1 {
				return cmdutil.FlagErrorf("--match accepts only one value for code search (either 'path' or 'file', not both)")
			}
...
func codeRun(opts *CodeOptions) error {
	...
	// Convert `owner` qualifier to `repo:owner/*` pattern for better compatibility
	// with the new GitHub code search engine. The `user:` qualifier is often ignored
	// when combined with other qualifiers, but `repo:owner/*` works reliably.
	// Skip this conversion for GHES hosts that may not support wildcard repo qualifiers.
	if len(opts.Query.Qualifiers.User) > 0 {
		...
		// FIXME: Remove this check once GHES supports wildcard repo qualifiers.
		if !ghauth.IsEnterprise(host) {
			// Convert each user qualifier to repo pattern
			for _, user := range opts.Query.Qualifiers.User {
				// Add repo pattern like "owner/*" to the Repo qualifier
				opts.Query.Qualifiers.Repo = append(opts.Query.Qualifiers.Repo, fmt.Sprintf("%s/*", user))
			}
			// Clear the User qualifier to avoid duplicate/conflicting filtering
			opts.Query.Qualifiers.User = nil
		}
	}
	
	// Note: filename, extension, and language qualifiers are passed through as-is
	// to match GitHub API code search semantics:
	// - filename: matches the file basename (e.g., filename:README matches README.md, README.txt)
	// - extension: matches file extension (e.g., extension:md matches .md files)
	// - language: filters by programming language (e.g., language:python matches .py files)
	// - path: matches the full file path (e.g., path:src/ matches files in src/ directory)
	//
	// These qualifiers are independent and can be combined. The GitHub API handles
	// them correctly, so no conversion is needed.
