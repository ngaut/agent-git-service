...
func (r PRFindRefs) Matches(baseBranchName, qualifiedHeadRef string) bool {
	...
	// When we couldn't determine the fork remote (owner is None), also match
	// on just the branch name part of cross-repo PR head labels (owner:branch).
	if !headMatches && r.qualifiedHeadRef.owner.IsNone() {
		if idx := strings.Index(qualifiedHeadRef, ":"); idx >= 0 {
			headMatches = qualifiedHeadRef[idx+1:] == r.QualifiedHeadRef()
		}
	}
