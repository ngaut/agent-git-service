...
func helperRun(opts *CredentialOptions) error {
	...
	if !isSupportedCredentialProtocol(wants["protocol"], wants["host"]) {
...
func isSupportedCredentialProtocol(protocol, host string) bool {
	switch protocol {
	case "https":
		return true
	case "http":
		return isLocalCredentialHost(host)
	default:
		return false
	}
}

func isLocalCredentialHost(host string) bool {
	hostOnly := host
	...
	hostOnly = strings.TrimSuffix(strings.ToLower(strings.TrimSpace(hostOnly)), ".")
	if hostOnly == "localhost" || hostOnly == "github.localhost" || strings.HasSuffix(hostOnly, ".localhost") {
		return true
	}
