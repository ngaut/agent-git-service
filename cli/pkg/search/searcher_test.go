...
func TestSearcherCode(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "searches code with filename qualifier",
			query: Query{
		...
		{
			name: "searches code with extension qualifier",
			query: Query{
				Keywords: []string{"test"},
				Kind:     "code",
				Limit:    30,
		...
		{
			name: "searches code with language and filename qualifiers",
			query: Query{
			...
			httpStubs: func(reg *httpmock.Registry) {
				reg.Register(
					httpmock.QueryMatcher("GET", "search/code", url.Values{
						"page":     []string{"1"},
						"per_page": []string{"30"},
						"q":        []string{"main filename:main.go language:go"},
					}),
					httpmock.JSONResponse(map[string]interface{}{
						"incomplete_results": false,
						"total_count":        1,
...
func TestSearcherCommits(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "paginates results with multi-word query tokenized for commits",
			query: Query{
				Keywords: []string{"keyword with whitespace"},
				Kind:     "commits",
				Limit:    30,
