...
func TestStandardSearchString(t *testing.T) {
	tests := []struct {
		...
		{
			name: "commit search splits multi-word keywords into tokens",
			query: Query{
				Kind:     KindCommits,
		...
		{
			name: "commit search with author-name qualifier",
			query: Query{
				Kind: KindCommits,
				Qualifiers: Qualifiers{
					AuthorName: "Jane Doe",
			...
			out: `author-name:"Jane Doe"`,
		},
		{
			name: "commit search with author-email qualifier",
			query: Query{
				Kind: KindCommits,
				Qualifiers: Qualifiers{
			...
			out: `author-email:jane@example.com`,
		},
		{
			name: "commit search with multi-token keywords and author qualifiers",
			query: Query{
				Kind:     KindCommits,
				Keywords: []string{"fix memory leak"},
				Qualifiers: Qualifiers{
					AuthorName:  "John Smith",
					AuthorEmail: "john@example.com",
				},
			},
			out: `fix memory leak author-email:john@example.com author-name:"John Smith"`,
		},
		{
			name: "commit search with immutable multi-word keywords",
			query: Query{
				Kind:              KindCommits,
				ImmutableKeywords: "performance optimization",
			},
...
func TestAdvancedIssueSearchString(t *testing.T) {
	tests := []struct {
		...
		// Issue #521: Exact phrase matching with in:body qualifier
		{
			name: "issue #521 - exact phrase in body",
			query: Query{
				Keywords: []string{"exact phrase"},
				Qualifiers: Qualifiers{
					In:   []string{"body"},
					Type: "issue",
				},
			},
			out: `"exact phrase" in:body type:issue`,
		},
		{
			name: "issue #521 - exact phrase in title and body",
			query: Query{
			...
			out: `"search terms" (in:body OR in:title) type:issue`,
		},
		{
			name: "issue #521 - boolean OR with in:body still uses parentheses",
			query: Query{
				Keywords: []string{"bug", "OR", "feature"},
				Qualifiers: Qualifiers{
					In:   []string{"body"},
					Type: "issue",
				},
			},
			out: `( bug OR feature ) in:body type:issue`,
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
