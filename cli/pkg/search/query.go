...
func (q Query) StandardSearchString() string {
	...
	if q.ImmutableKeywords != "" {
		if q.Kind == KindCommits {
			// For commit search, split immutable keywords into individual tokens
			keywords = formatKeywordsForCommits([]string{q.ImmutableKeywords})
		} else {
			keywords = []string{q.ImmutableKeywords}
		}
	} else if q.Kind == KindCommits {
		// For commit search, split multi-word keywords into individual tokens
		if ks := formatKeywordsForCommits(q.Keywords); len(ks) > 0 {
			keywords = ks
		}
...
func (q Query) AdvancedIssueSearchString() string {
	...
	if qualifiers != "" && keywords != "" {
		// Only wrap keywords in parentheses if they contain boolean operators
		// to prevent operator leakage. Simple keywords and phrases don't need
		// parentheses and match GitHub API behavior for exact phrase matching.
		if containsBooleanOperator(keywords) {
...
// containsBooleanOperator checks if the query string contains boolean operators
// that require grouping with parentheses to prevent operator leakage.
func containsBooleanOperator(query string) bool {
	// Check for standalone boolean operators (case-insensitive)
	// We look for operators surrounded by spaces or at word boundaries
	// to avoid matching them as part of quoted phrases or other text.
...
// formatKeywordsForCommits formats keywords for commit search, splitting multi-word
// phrases into individual tokens to match GitHub's commit search semantics.
func formatKeywordsForCommits(ks []string) []string {
	result := make([]string, 0, len(ks))
	for _, k := range ks {
		before, after, found := strings.Cut(k, ":")
		if !found {
			// Split multi-word keywords into individual tokens for commit search
			words := strings.Fields(k)
			for _, word := range words {
				result = append(result, quote(word))
			}
		} else {
			result = append(result, fmt.Sprintf("%s:%s", before, quote(after)))
		}
	}
	return result
}
